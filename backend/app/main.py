...
def create_feedback():
    ...
    if form.validate():
        ...
        if not relationship:
            return jsonify({"msg": "Only allowed to give feedback to your meeting"}), 401
...
def get_wishlist():
    ...
    items = meeing.get_wishlist_items.paginate(page, app.config['PER_PAGE'], False)
    ret = {'msg': 'Success',
           'data': [{'item id': it.id,
                     'product id': it.product_id,
                     'amount': it.amount,
                     'total price': it.price} for it in items.iter_pages()]}
...
def delete_wishlist_item():
    ...
    if not item:
        return jsonify({"msg": "Failed to find this item"}), 401
    relationship = MU_Relationship.query.filter_by(meeting_id=item.meeting_id, user_id=get_jwt_identity()).first()
    if not relationship:
        return jsonify({"msg": "Only allowed to delete item in your meeting"}), 401
